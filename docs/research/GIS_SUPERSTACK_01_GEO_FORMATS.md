...
FlatGeobuf is a compact binary spatial format based on FlatBuffers with fast random access. It is suitable for large vector files and streaming. It is part of the 2026 "Super-Stack" for high-performance client interchange.
...
GeoParquet stores spatial data in columnar Parquet format with geometry columns (WKB/Geometric typed). It is the 2026 gold standard for analytics and DuckDB-Spatial offloading.
...
Rollback / fallback note: Browsers cannot read Parquet directly without DuckDB-Wasm or similar; convert GeoParquet to GeoJSON or to vector tiles (MVT/PMTiles) for standard MapLibre use.
