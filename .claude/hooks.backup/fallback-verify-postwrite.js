...
const HAS_LIVE = /fetch\(|supabase\.(from|rpc)|axios\.|got\(|http\.get/i.test(
  content,
);
...
const HAS_CACHED =
  /api_cache|getCachedResponse|getCached|cachedData|cache\.get/i.test(content);
if (!HAS_CACHED) missing.push("CACHED (api_cache table)");

// Tier 3 — MOCK: public/mock/ fallback file reference
const HAS_MOCK = /public\/mock|mock\/.*\.geojson|readMockFile|MOCK_DATA/i.test(
  content,
);
