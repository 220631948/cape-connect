...
## What It Does
Reads the active agent definition from `.github/copilot/agents/` (and `.github/agents/` for fleet-level docs agents) and compares the files you've modified in this session against the agent's ALLOWED TOOLS AND FILES section.

## Procedure
1. Identify the current agent (from conversation context or ask)
2. Read `.github/copilot/agents/[agent-name].agent.md` (or `.github/agents/[agent-name].agent.md`)
