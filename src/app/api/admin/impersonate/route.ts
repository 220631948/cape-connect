...
function resolveDurationSeconds(rawDuration: unknown): number {
  const parsed = Number(rawDuration);
  if (!Number.isFinite(parsed)) return 15 * 60;
  return Math.min(Math.max(Math.round(parsed), 60), 15 * 60);
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const durationSeconds = resolveDurationSeconds(body?.duration_seconds);
